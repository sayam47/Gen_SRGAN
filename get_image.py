...
parser.add_argument('--output_path' , type=str , default='/home/sayam.choudhary.cse17.iitbhu/test_output/output_images' , help = 'location of test folder')
...
def plot_image(generators , dataloader, model_names):
	for i,imgs in tqdm(enumerate(dataloader)):
		...
		for j in range(imgs_lr.shape[0]):
			...
			file_name = os.path.join(args.output_path ,'generated_image_' + str(batches_done) + '.png')
			csv_file_name = os.path.join(args.output_path ,'generated_image_' + str(batches_done) + '.csv')
			...
			grid_image = torchvision.utils.make_grid(final_image, nrow=3)
