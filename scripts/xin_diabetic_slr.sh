...
output_path="/scratch/sayam.choudhary.cse17.iitbhu/output_xin_diabetic_slr"

# VGG(L1) * 1 + VGAN * 0.005 + L1 * 0.01

python3 train.py --train_data_path "$train_data_path" --test_data_path "$test_data_path" --epochs $epochs --plot_every $plot_every --print_every $print_every --n_threads $n_threads --output_path "$output_path" --batch_size $batch_size --debug $debug --output_size 256x256 --warmup_batches 0 --method M1 --gan VGAN --vgg_criterion L1 --weight_gan 0.005 --weight_vgg 1 --include_l1 1 --weight_l1 0.01 --lr 0.00003
