		...
		for i,imgs in tqdm(enumerate(dataloader)):
			...
			if args.method == 'M4':
				...
				if (epoch - start_epoch) < args.loss_mem:
					...
					weight_vgg = losses_log[-args.loss_mem: , 1].mean() * args.mem_vgg_weight
					weight_bce = losses_log[-args.loss_mem: , 2].mean() * args.mem_bce_weight
					weight_hv = losses_log[-args.loss_mem: , 3].mean() * args.mem_hv_weight
			...
			if args.method == 'M7':
				if (epoch - start_epoch) >= args.loss_mem:
					weight_dis = losses_log[-args.loss_mem: , 5].mean() * args.mem_bce_weight
